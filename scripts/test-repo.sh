...
check "Voidbleed repo configured"              grep -q '^repository=https://void.7mm.ir/voidbleed$' "$t/usr/share/xbps.d/10-voidbleed.conf"
...
check "greetd service releases the splash"     grep -q 'plymouth quit --retain-splash' "$t/etc/sv/greetd/run"
check "greetd service still starts greetd"     grep -q '^exec greetd$' "$t/etc/sv/greetd/run"
check "the greeter does not try to quit it"    bash -c "! grep -q 'plymouth quit' '$t/usr/bin/voidbleed-greeter-session'"
